        ...
        <div class="card-body">
            <form method="post" action="{{route('product.update',$product->id)}}">
                ...
                <div class="form-group">
                    ...
                    <select name="cat_id" id="cat_id" class="form-control">
                        ...
                        @foreach($categories as $key=>$parent_cat)
                            <option @if($product->cat_id == $parent_cat->id)  selected
                                    @endif value='{{$parent_cat->id}}'>{{$parent_cat->title}}</option>
                            @php
                                $child_cats = App\Models\Category::where('parent_id', $parent_cat->id)->orderBy('title','ASC')->get();
                            @endphp
                            @foreach($child_cats as $i =>$child_cat)
                                <option @if($product->cat_id == $child_cat->id)  selected
                                        @endif value='{{$child_cat->id}}'>  - {{$child_cat->title}}</option>

                                @php
                                    $child_cats2 = App\Models\Category::where('parent_id', $child_cat->id)->orderBy('title','ASC')->get();
                                @endphp
                                @foreach($child_cats2 as $child_cat2)
                                    <option @if($product->cat_id == $child_cat2->id)  selected
                                            @endif value='{{$child_cat2->id}}'>     - {{$child_cat2->title}}</option>
                                    @php
                                        $child_cats3 = App\Models\Category::where('parent_id', $child_cat2->id)->orderBy('title','ASC')->get();
                                    @endphp
                                    @foreach($child_cats3 as $child_cat3)
                                        <option @if($product->cat_id == $child_cat3->id)  selected
                                                @endif value='{{$child_cat3->id}}'>
                                                    - {{$child_cat3->title}}</option>
                                    @endforeach
                ...
                <div class="form-group">
                    ...
                    <input id="price_logo" type="number" name="price_logo" placeholder="Enter price" value="{{$product->price_logo}}"
                           class="form-control">
                    @error('price_logo')
                    <span class="text-danger">{{$message}}</span>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <input id="price_customize" type="number" name="price_customize" placeholder="Enter price"
                           value="{{$product->price_customize}}"
                           class="form-control">
